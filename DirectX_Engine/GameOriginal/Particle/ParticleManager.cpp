...
void ParticleManager::Update()
{
	...
	constMap->color = XMFLOAT4(1, 0.5, 0, 1);//色指定(R G B A)
	constBuff->Unmap(0, nullptr);
}

void ParticleManager::BlueUpdate()
{
	HRESULT result;
	...
	constMap->mat = camera->GetViewProjectionMatrix();
	constMap->matBillboard = camera->GetBillboardMatrix();
	constMap->color = XMFLOAT4(0, 0, 1, 1);//色指定(R G B A)
	constBuff->Unmap(0, nullptr);
}
...
void ParticleManager::CreateParticles(XMFLOAT3 pos)
{
	for (int i = 0; i < 10; i++) {
		// X,Y,Z全て[-5.0f,+5.0f]でランダムに分布
		const float rnd_pos = 0.0f;
		
		pos.x = pos.x+(float)rand() / RAND_MAX * rnd_pos - rnd_pos / 2.0f;
		pos.y = pos.y+(float)rand() / RAND_MAX * rnd_pos - rnd_pos / 2.0f;
		pos.z = pos.z+(float)rand() / RAND_MAX * rnd_pos - rnd_pos / 2.0f;
